...
const { data: experiencia, loading, addItem, updateItem, deleteItem } = useFirestoreCollection('experiencia', {
  autoFetch: true,
  realtime: true
});
...
const abrirNuevo = () => {
  ...
  formulario.value = {
    ...
    competencias: '',
    tecnologiasTexto: '',
    orden: (experiencia.value?.length || 0) + 1
  };
...
const guardarExperiencia = async () => {
  if (!formulario.value.Name.trim()) {
    errorForm.value = 'El nombre de la empresa / institución es obligatorio.';
    return;
  }
  ...
  const payload = {
    ...
    Cargo: formulario.value.Cargo.trim(),
    Descripcion: formulario.value.Descripcion.trim(),
    competencias: formulario.value.competencias.trim(),
...
const confirmarEliminar = async (id, name) => {
  if (confirm(`¿Estás seguro de eliminar el registro de experiencia "${name}"?`)) {
    try {
      await deleteItem(id);
    } catch (err) {
...
<template>
  <div class="bg-[rgba(var(--gris-oscuroRGB),0.7)] border border-gray-800 rounded-2xl p-6 md:p-8 backdrop-blur-md">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-gray-800">
      <div>
        <h2 class="text-xl font-bold font-['Rubik'] text-[var(--gris-claro)] flex items-center gap-2 m-0 text-left">
          <i class="bi bi-briefcase text-[var(--verde-cian)]"></i>
          Gestión de Experiencia Laboral
        </h2>
        <p class="text-xs text-gray-400 mt-1">Administra tus roles y trayectoria profesional</p>
      </div>
    ...
    <div v-if="loading && (!experiencia || experiencia.length === 0)" class="py-12 text-center text-gray-400">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-[var(--verde-cian)] mt-0.5 text-left" v-html="item.Cargo"></p>
          </div>
        </div>
      ...
      <div class="w-full max-w-2xl bg-[var(--gris-oscuro)] border border-gray-700 rounded-2xl p-6 md:p-8 max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="flex items-center justify-between pb-3 mb-5 border-b border-gray-700">
          <h3 class="text-lg font-bold font-['Rubik'] text-[var(--gris-claro)] m-0">
            {{ editandoId ? 'Editar Experiencia' : 'Nueva Experiencia' }}
          </h3>
        ...
        <form @submit.prevent="guardarExperiencia" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-300 mb-1">Empresa / Institución *</label>
              <input 
                v-model="formulario.Name" 
                type="text" 
                required 
                placeholder="Ej: Colegio Guayamuri"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Cargo / Rol</label>
            <input 
              v-model="formulario.Cargo" 
              type="text" 
              placeholder="Ej: Full-Stack Developer & Coordinador STEM"
              class="w-full bg-[var(--fondo-negro)] border border-gray-700 focus:border-[var(--verde-cian)] rounded-xl py-2 px-3 text-xs text-[var(--gris-claro)] outline-none"
            />
          </div>

          <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Descripción de Funciones</label>
            <textarea 
              v-model="formulario.Descripcion" 
              rows="3" 
              placeholder="Detalle de actividades desempeñadas..."
              class="w-full bg-[var(--fondo-negro)] border border-gray-700 focus:border-[var(--verde-cian)] rounded-xl py-2 px-3 text-xs text-[var(--gris-claro)] outline-none"
            ></textarea>
          </div>

          <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Competencias Desarrolladas</label>
            <textarea 
              v-model="formulario.competencias" 
              rows="2" 
              placeholder="Habilidades clave adquiridas..."
              class="w-full bg-[var(--fondo-negro)] border border-gray-700 focus:border-[var(--verde-cian)] rounded-xl py-2 px-3 text-xs text-[var(--gris-claro)] outline-none"
            ></textarea>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                type="text" 
                placeholder="Python, Arduino, Vue, Git"
                class="w-full bg-[var(--fondo-negro)] border border-gray-700 focus:border-[var(--verde-cian)] rounded-xl py-2 px-3 text-xs text-[var(--gris-claro)] outline-none"
              />
            </div>
            <div>
